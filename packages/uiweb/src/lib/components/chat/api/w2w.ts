import { Feeds, MessageIPFSWithCID, User } from '../types';

let BASE_URL = 'https://backend.epns.io/apis'

export const getInbox = async (did: string): Promise<Feeds[] | null> => {
    let retry = 0;
    for (let i = 0; i < 3; i++) {
        try {
...
export const getIntents = async (did: string): Promise<Feeds[] | null> => {
    let retry = 0;
...
export const getUser = async ({ did = '', caip10 = '' }: { did?: string; caip10?: string }): Promise<User | null> => {
    let retry = 0;
